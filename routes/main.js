...
var project = require('../routesmodels/mainmodels'); // Project configuration
var apimodels = require('../routesmodels/routesmodels'); //used for GET/INSERT/UPDATE/DELETE
    ...
    if (route.type === "get") {
        router.get("/" + route.route, ensureAuthenticated, function (req, res) {
            if (service.checkRights(req.user, route.rol)) {
                ...
                apimodels.models[route.model_function](project.model.database, route.collection, query.query, query.sort, function (err, items) {
        ...
        router.post("/" + route.route, ensureAuthenticated, function (req, res) {
            apimodels.models[route.model_function](project.model.database, route.collection, req.body, function (err, items) {
                res.status(201);
        ...
        router.post("/" + route.route + "/:_id", ensureAuthenticated, function (req, res) {
            ...
            apimodels.models[route.model_function](project.model.database, route.collection, query, function (err, items) {
                res.status(201);
                res.redirect('/');
            });
        });
        router.delete("/" + route.route + "/:_id", ensureAuthenticated, function (req, res) {
            ...
            apimodels.models.delete(project.model.database, route.collection, {_id: _id}, function (err, items) {
                res.setHeader('Content-Type', 'application/json');
                res.send(items);
            });
        });
        router.put("/" + route.route + "/:_id", ensureAuthenticated, function (req, res) {
            ...
            apimodels.models.put(project.model.database, route.collection, query, update, function (err, items) {
                res.setHeader('Content-Type', 'application/json');
                res.send(items);
            });
        });
